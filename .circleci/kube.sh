...
docker build --rm=false --build-arg DEP_VERSION=$DEP_VER -t coderdojo/coolest-platform:"${GIT_SHA1}-$(date +%s)" .
docker login -u "$DOCKER_USER" -p "$DOCKER_PASS" -e "$DOCKER_EMAIL"
docker push coderdojo/coolest-platform:"${GIT_SHA1}-$(date +%s)"
...
./kubectl --namespace=coolest-namespace patch deployment coolest-platform -p '{"spec":{"template":{"spec":{"containers":[{"name":"coolest-platform","image":"coderdojo/coolest-platform:'"${GIT_SHA1}-$(date +%s)"'"}]}}}}'
