...
# Build and push
# INT
docker build --rm=false -t coderdojo/coolest-platform:"${GIT_SHA1}-${TIMESTAMP}" --build-arg "EVENT_SLUG=int-2020" --build-arg "GOOGLE_ANALYTICS_PROPERTY_ID=UA-112027238-2" .
...
# USA
docker build --rm=false -t coderdojo/coolest-platform:usa -t coderdojo/coolest-platform:usa-"${GIT_SHA1}-${TIMESTAMP}" --build-arg "EVENT_SLUG=usa-2020" --build-arg "GOOGLE_ANALYTICS_PROPERTY_ID=UA-112027238-4" .
...
# UK
docker build --rm=false -t coderdojo/coolest-platform:uk -t coderdojo/coolest-platform:uk-"${GIT_SHA1}-${TIMESTAMP}" --build-arg "EVENT_SLUG=uk-2020" --build-arg "GOOGLE_ANALYTICS_PROPERTY_ID=UA-112027238-5" .
