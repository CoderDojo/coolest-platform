...
describe('projects controllers: setSeatingPerCategory', () => {
  ...
  it('shoud do the age split if the category is SC', async () => {
    ...
    expect(controllers.setSeating.getCall(0)).to.have.been.calledWith('SC', ['age', '<', 13], 100);
    expect(controllers.setSeating.getCall(1)).to.have.been.calledWith('SC', ['age', '>', 12], 100 + sizeSeed);
  });
});
